...
Background:
    ...
    1. Filters to genome-wide significant SNVs on the fly (to avoid OOM on
       memory-limited login nodes — only ~20k SNPs are kept, not all 7.6M)
    2. Removes indels (keeps only single-base substitutions, required by AlphaGenome)
...
def read_and_filter_gwas(filepath, p_threshold=P_THRESHOLD):
    ...
    (login nodes on Hoffman2 have limited RAM — only ~20k significant
    SNVs are kept).
    """
    snvs = []
    total = 0
    ...
    with gzip.open(filepath, 'rt') as f:
        for line in f:
            ...
            # Remove indels (AlphaGenome requires single-base substitutions)
            a1 = row['A1'].upper()
            a2 = row['A2'].upper()
            if not is_snv(a1, a2):
                n_indels += 1
...
def main():
    ...
    snvs, total_snps, n_sig, n_indels = read_and_filter_gwas(INPUT_FILE)
    print(f"Total SNPs in file: {total_snps:,}")
    print(f"Genome-wide significant (p < {P_THRESHOLD}): {n_sig:,}")
    print(f"After removing indels: {len(snvs):,} SNVs ({n_indels:,} indels removed)")

    # === Step 1: Document MHC dominance ===
    ...
    print(f"Top 50 SNVs by p-value: {top50_chr6}/50 are on chr6")

    # Find rank of first non-chr6 SNP
    for i, s in enumerate(snvs_sorted):
        if s['chr'] != MHC_CHR:
            print(f"First non-chr6 SNV: rank #{i+1} ({s['rsid']}, "
    ...
    print(f"Saved top 50 SNVs to {mhc_out}")

    # === Step 2: Exclude MHC and clump ===
